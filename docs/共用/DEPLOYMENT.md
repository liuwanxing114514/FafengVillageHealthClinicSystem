...
> 2026-07-09 起不再实施。AI 统一使用硅基流动 / DeepSeek 等 API；`LocalAiProvider` 保持占位。

~~验收清单（归档）~~：

- ~~`provider=local` 时 Agent/整理可用~~
- ~~外网断开、本地服务正常时基础+AI 可用~~

### v2.6 导出 / PDF

- [x] 库存流水导出 Excel 与筛选一致
- [x] 病历可保存 PDF
...
2. **配置 `.env`**：合并 `CLINIC_EMBEDDING_*`（见第六节示例）。推荐硅基流动 `BAAI/bge-m3`（1024 维）。
3. **重启 backend** 使配置生效：
   ```powershell
   cd D:\clinic
