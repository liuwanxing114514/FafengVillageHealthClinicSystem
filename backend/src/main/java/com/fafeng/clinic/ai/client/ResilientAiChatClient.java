...
public class ResilientAiChatClient implements AiChatClient {
    ...
    public String chatCompletion(String systemPrompt, String userMessage) {
        return chatCompletion(systemPrompt, userMessage, false);
    }

    @Override
    public String chatCompletion(String systemPrompt, String userMessage, boolean skipDesensitization) {
        requireConfigured();
        return invokeWithFailover(channel -> {
            var client = skipDesensitization
                    ? chatChannelFactory.buildClientWithoutAdvisor(channel.config())
                    : channel.chatClient();
    ...
    public String chatWithTools(String systemPrompt, String userMessage, Object toolSource) {
        return chatWithTools(systemPrompt, List.of(), userMessage, toolSource);
    }

    @Override
    public String chatWithTools(String systemPrompt, List<Message> history, String userMessage, Object toolSource) {
        requireConfigured();
        return invokeWithFailover(channel -> {
            var prompt = channel.chatClient().prompt().system(systemPrompt);
            if (history != null && !history.isEmpty()) {
                prompt = prompt.messages(history);
            }
            return prompt.user(userMessage).tools(toolSource).call().content();
        });
    }
